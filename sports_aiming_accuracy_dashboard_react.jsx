...
export type Attempt = {
  id: string;
  t: number; // attempt index/time
  hit: boolean; // on target or not
  err: number; // radial error in cm
  x: number; // horizontal deviation (cm), +right/-left
  y: number; // vertical deviation (cm), +high/-low
  releaseDeg?: number; // release angle in degrees (if available)
  speed?: number; // initial speed m/s (optional)
};

// -------------------------
// Mock seed data (basketball free throws)
// -------------------------
const SEED: Attempt[] = Array.from({ length: 60 }).map((_, i) => {
...
function rollingAccuracy(data: Attempt[], window = 10) {
  const out: { t: number; acc: number }[] = [];
...
function toBins(data: Attempt[], size = 6) {
  const binKey = (x: number, y: number) => `${Math.round(x / size)}_${Math.round(y / size)}`;
...
export default function SportsAccuracyDashboard() {
  ...
  function parseCSV(text: string) {
    ...
    const get = (obj: Record<string, string>, k: string) => Number(obj[k] ?? "");
    const out: Attempt[] = lines.map((line, i) => {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-zinc-50 to-zinc-100 dark:from-zinc-950 dark:to-zinc-900 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="text-xs text-zinc-500 text-center">
...
function CoachingCues({ metrics, sport }: { metrics: any; sport: string }) {
  const cues: string[] = [];
...
function TargetPlane({ attempts, targetRadius }: { attempts: Attempt[]; targetRadius: number }) {
